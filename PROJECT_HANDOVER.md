...
**代码位置**（本地工作目录 ~/dsh/jizhang）：
- 记账本主后端：server/（入口 server/index.js，路由 server/routes/）
- AI 后端：server-ai/（入口 server-ai/index.js，路由 server-ai/routes/ai.js）
- 工资后端：server-salary/（入口 server-salary/index.js，路由 server-salary/routes/salary.js）
- 三国杀后端：server-sgs/（入口 server-sgs/index.js，路由 server-sgs/routes/sgs.js）
- 记账本前端：public/
- 主站门户：portal/
- 微信小程序：miniprogram/（未联调）
...
- /api/ai/* → AI 后端（:3001，jizhang-ai）
- /api/salary/* → 工资后端（:3002，jizhang-salary）
- /api/sgs/* → 三国杀大厅代理（:3003，jizhang-sgs）
- /api/* → 记账本主后端（:3000）
- /sgs-api/* 和 /sgs-ws/* → 三国杀游戏服务器（:8085/:8080）

> Nginx 分流顺序：/api/ai/、/api/salary/、/api/sgs/ 必须排在 /api/ 之前，否则会全落到主后端。

### 服务器关键路径
- 主站文件：/opt/www/（index.html、ai.html、wuxia.html、salary.html、sgs.html、katex/）
- 记账本：/opt/jizhang/public/（前端）+ /opt/jizhang/server/（主后端）
- 独立后端：/opt/jizhang/server-ai/ + server-salary/ + server-sgs/
- 数据库：/opt/jizhang/data/jizhang.db（4 个后端共用同一个库）
...
### PM2 进程（后端拆分后共 4 个业务进程）
- jizhang-api（记账本主后端，端口 3000，最大内存 300M）
- jizhang-ai（AI 助手 + 江湖模拟器，端口 3001）
- jizhang-salary（工时工资，端口 3002）
- jizhang-sgs（三国杀大厅代理，端口 3003）
- sgs-server（三国杀游戏服务器，端口 8080/8085）
- pm2-logrotate（日志轮转）

> 4 个后端共用同一 SQLite 库，登录态（sessions 表）天然互通；AI 后端记账工具通过 HTTP 调主后端（JZ_JIZHANG_API，默认 http://127.0.0.1:3000），不直连记账表。详见 BACKEND_SPLIT.md。
...
**独立后端（各自进程，共用同一 SQLite 库）**

| 进程 | 路由文件 | 前缀 | 功能 |
|---|---|---|---|
| jizhang-ai (:3001) | server-ai/routes/ai.js | /api/ai | AI 对话（/chat、/providers、/public-chat 免登录）、江湖存档、会话同步 |
| jizhang-salary (:3002) | server-salary/routes/salary.js | /api/salary | 工时工资模块 |
| jizhang-sgs (:3003) | server-sgs/routes/sgs.js | /api/sgs | 三国杀房间代理 |
...
- 后端：server-salary/routes/salary.js（独立进程 :3002）
...
6. 部署流程：本地改 → git commit → push（可能需 -c http.sslBackend=openssl 或直连）→ 服务器 deploy.sh（拉代码 + 重启 4 个后端 + 逐端口健康检查）
...
- 后端已拆分为 4 个独立进程（commit 1e08a95）
- 已删除拆分后遗留的孤儿路由文件：server/routes/ai.js、salary.js、sgs.js（不再被任何代码引用）
- 4 个后端全局限流统一为每 IP 每分钟 200 次
- 交接时先 git status / git log 确认最新状态
