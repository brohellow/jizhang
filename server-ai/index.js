...
// 免登录公用 AI（/api/ai/public-chat、/api/ai/save-state、/api/ai/load-state）
app.use('/api/ai', aiPublicRouter);
// 登录 AI（/chat、/providers、/conversations 等）
...
app.get('/api/health', (req, res) => {
  try {
    ...
    res.json({ ok: true, service: 'ai-backend', db: 'ok', time: new Date().toISOString() });
  } catch (e) {
    res.status(500).json({ ok: false, service: 'ai-backend', error: e.message });
  }
});
...
const port = config.aiPort;
const server = app.listen(port, () => {
  console.log('AI 后端已启动: http://localhost:' + port + '（含 AI 助手 + 江湖模拟器）');
});
